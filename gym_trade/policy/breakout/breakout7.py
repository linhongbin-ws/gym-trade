...
class Policy(BasePolicy):
    def init_policy(self, **kwargs):
        ...
        self.buysignal_skipnum = 0 
        self.max_buycnt = 10
        self.max_loss = 0.03
        self.exit_pullback = 0.1
        self.min_volumeprice = 0
        ...
        self.start_trade_timestep = 16
        self.highest_prv = 0
        self.cool_down_cnt= 0
        self.cool_down_size= 5
        self.hot_cnt = 0
        self.hot_size = 80
        self.skip_next_buy = False
        if self.gui:
            self.env.gui_init()
    ...
    def __call__(self, obs, **kwargs):
        ...
        if not obs['break_high'] and not self.break_high_prv:
            self.cool_down_cnt +=1
        if obs['break_high']:
            self.hot_cnt+=1
        ...
        if self.low_liquid is None:
            if obs['high'] == obs['low'] or (self.min_volumeprice > obs['volume']*obs['close']):
        ...
        # if not obs['break_high'] and self.break_high_prv and self.cool_down_cnt>=self.cool_down_size:
        #     print(self.env.timestep)
       
        if not obs['break_high'] and self.break_high_prv and self.cool_down_cnt>=self.cool_down_size \
            and self.hot_cnt >= self.hot_size\
            ...
            #   or ( (self.pnl_highest-pnl) // self.exit_pullback -  (self.pnl_highest-self.pnl_prv) // self.exit_pullback) >= 1
              or (pnl < -self.max_loss)
              or not obs["break_high_or"] 
        ...
        if not obs['break_high'] and self.break_high_prv:
            self.cool_down_cnt = 0
